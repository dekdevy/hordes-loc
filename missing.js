...
const compile = (obj, lang, currentType, metrics, path)=>{
    ...
    if(typeof value  === 'object') {
      ...
      if(Object.entries(value).some(v => typeof v[1] === 'string')) {
        ...
        // i guess they never miss huh
          ...
          if(value['$$'] === undefined) {
            console.warn(`The ${currentType} translation is missing in ${path} → ${key}`)
            metrics.miss++
          }
          lang[key] = fallback
        }
        ...
        if (path!=undefined)
          compile(value, lang[key] = (value.length ? [] : {}),
            currentType, metrics, path + ' → '+ key )
        else
          compile(value, lang[key] = (value.length ? [] : {}),
            currentType, metrics, key)
